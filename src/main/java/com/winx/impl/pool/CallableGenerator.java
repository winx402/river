...
import com.winx.exception.UnknownResultTypeException;
...
public class CallableGenerator extends MethodPointerHandler.MethodPointerLimit5 {
    ...
    public static Callable generateCallable() {
        ReturnType returnType = getReturnType();
        if (returnType == ReturnType.VOID) {
            return new RunnableAdapter(new RunnableCarrier(), null);
        }
        if (returnType == ReturnType.UNKNOW) throw new UnknownResultTypeException("unknown result type for method : " + methodName());
